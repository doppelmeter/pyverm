    ...
    def _connect(self, *,retries=10):
        ...
        while counter < retries:
            ...
            try:
                ...
                logger.warning("\tConnecting to %s (Port %s) failed in attempt %s\ttime out",str(self.tcp_ip), str(self.tcp_port),str(counter))
                time.sleep(0.1)
            except OSError as err:
                logger.warning("\tConnecting to %s (Port %s) failed in attempt %s \t%s",str(self.tcp_ip), str(self.tcp_port),str(counter), err)
                time.sleep(0.1)
            else:
    ...
    def request(self, rpc, *parameterlist):
        ...
        while counter < 2:
            ...
            try:
                ...
                val_list = list(map(convert, val_list))
                return val_list
        logger.error("\tRequest to %s (Port %s) failed", str(self.tcp_ip), str(self.tcp_port))
