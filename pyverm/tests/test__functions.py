...
class Test_distance:
    ...
    def test_distance_returns_correct_value(self):
        ...
        assert var_1 == pytest.approx(Decimal(100), abs=Decimal(1e-7))
        var_2 = _functions.distance((600000, 200000), (2600000, 1200000))
        assert var_2 == pytest.approx(Decimal(2236067.977499789696409173669), abs=Decimal(1e-7))
        var_3 = _functions.distance((0, 1), (0, 1200001))
        assert var_3 == Decimal(1200000)

class Test_azimuth:
    def test_azimuth_between_the_same_points(self):
        point = (10,10)
        var = _functions.distance(point,point)
        assert var == 0

    def test_azimuth_returns_decimal(self):
        var =  _functions.distance((0,0),(10,10))
        assert isinstance(var, Decimal)

    def test_azimuth_returns_correct_value(self):
        ...
        for point, azimuth in test_values:
            ...
            assert var == pytest.approx(Decimal(azimuth), abs=Decimal(1e-7))
...
class Test_cartesian:
    def test_cartesian_returns_correct_value(self):
        ...
        for point, azimuth in test_values:
            ...
            assert var_y == pytest.approx(Decimal(y), abs=Decimal(1e-7))
            assert var_x == pytest.approx(Decimal(x), abs=Decimal(1e-7))
...
class Test_polar:
    def test_cartesian_returns_correct_value(self):
        ...
        for point, azimuth in test_values:
            ...
            assert var_dist == pytest.approx(Decimal(1), abs=Decimal(1e-7))
            assert var_azimuth == pytest.approx(Decimal(azimuth), abs=Decimal(1e-7))
...
class Test_abriss:
    def test_abriss_returns_correct_value(self):
        ...
        assert var == pytest.approx(Decimal(50), abs=Decimal(1e-7))
...
class Test_free_station:
    def test_abriss_returns_correct_value(self):
        ...
        assert var_ori == pytest.approx(Decimal(50), abs=Decimal(1e-7))
        assert var_y == pytest.approx(Decimal(2600000), abs=Decimal(1e-7))
        assert var_x == pytest.approx(Decimal(1200000), abs=Decimal(1e-7))
